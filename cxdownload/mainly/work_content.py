...
def process_row(row):
    ...
    try:
        # 调用大模型API提取技术栈
        prompt = "从我给你们的岗位描述中摘录出来岗位职责或是工作内容，注意不是任职资格。返回格式：\n1. 工作内容1\n2. 工作内容2\n3. ..."
...
def main():
    ...
    with open(args.input, 'r', newline='', encoding='utf-8') as csvfile, \
        ...
        # 写入标题行
        writer.writerow(['职位名称', '工作地址','学历要求', '工作年限要求','招聘人数','薪资待遇','公司行业','公司性质','公司规模','融资阶段','招聘状态','职位类型','岗位描述','公司介绍','公司工商信息','简历详情页地址','更新日期','工作内容'])
        next(reader)  # 跳过标题行
